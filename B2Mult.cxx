...
pair< unsigned int, unsigned int > validateParams(int argc, char *argv[], unsigned int lambdas[]) {
  ...
  if (argc > 3)
    ...
    {
      cout << "Please call this program like \'B2Mult index iterations lambda1 lambda2...\' "
...
#pragma omp parallel for schedule(static) shared(lambdas, logtable, p, evalV, primZetaEval)
  for (unsigned long p1 = 0; p1 < p; ++p1) {
	...
	mpz_mul(p2big[i], p2big[0], p2big[i-1]);
      ...
      mpz_set_ui(chiArg, 0);
      // our polynomial is
      // 1 + p2 + p1*p2 + p2^2 + lambda*p1*p2^2 + p1^2*p2^2 + p1*p2^3 + p1^2*p2^3 + p1^2*p2^4
      ...
      mpz_addmul(chiArg, p1big[1], p2big[3]);
      for (unsigned int l = 0; l < lambdaLength; ++l) {
	mpz_set_ui(chiArgLambda, lambdas[l]);
	mpz_set_ui(zetaPower, 0);
	mpz_mul(chiArgLambda, chiArgLambda, p1big[0]);
	mpz_mul(chiArgLambda, chiArgLambda, p2big[1]);
...
void print2DV(const unsigned int lambdaLength, unsigned long p, mpc_t V[]) {
  ...
  sprintf (output_filename, "B2MultOut/b2mult%06lu", p);
