...
pair< unsigned int, unsigned int > validateParams(int argc, char *argv[], unsigned int lambdas[]) {
  ...
  if (argc > 3)
    ...
    {
      cout << "Please call this program like \'A3Mult index iterations lambda1 lambda2...\' "
...
#pragma omp parallel for schedule(static) shared(lambdas, logtable, p, evalV, primZetaEval)
  for (unsigned long p1 = 0; p1 < p; ++p1) {
    ull logLookup;
    mpz_t chiArg, chiArgLambda, zetaPower, p1p2p3big, temp;
    mpz_t p1big[2]; // to store powers of p1
    mpz_t p2big[2]; // to store powers of p2
    mpz_t p3big[2]; // to store powers of p3
    mpz_init(chiArg);
	...
	mpz_set_ui(p1p2p3big, (((ull) p1*p2*p3) % ((ull) p)) );
        ...
        mpz_mod_ui(chiArg, chiArg, p);
        mpz_addmul(chiArg, p2big[0], p3big[0]);
        mpz_mod_ui(chiArg, chiArg, p);
        mpz_addmul(chiArg, p2big[0], p1p2p3big);
        mpz_mod_ui(chiArg, chiArg, p);
	mpz_set_ui(temp, 0);
